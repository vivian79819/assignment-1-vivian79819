 ...
 * <p>
 * Write your name and UPI here: Vivian Hsiao-Wei Yeh vyeh766.
 */
public class AssignmentOnePartOne {
    ...
    public String getThreeLettersInCapital(String text, int index) {
        ...
        if (index < text.length() - 2) {
            String threeLettersInCapital = text.substring(index, text.length());
            return threeLettersInCapital.toUpperCase();
        }
        if (index > text.length()) {
            return " ";
        }
        if (index >= text.length() - 2) {
            String lettersAfterIndex = text.substring(index, text.length());
            return lettersAfterIndex.toUpperCase();
        }
    ...
    public int medianOfThreeInts(int numOne, int numTwo, int numThree) {
        // Answer here
       int median = Math.max(Math.min(numOne, numTwo), Math.min(Math.max(numOne, numTwo), numThree));
        return median;
        //
    }
}